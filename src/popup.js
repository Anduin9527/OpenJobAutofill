...
els.copyDebugLogBtn.addEventListener("click", () => {
  void copyDebugLog();
});
...
async function copyDebugLog() {
  try {
    ...
    if (!snapshot) {
      setStatus("还没有调试日志，请先运行一次“开始填写”。", true);
      return;
    }
    await writeClipboardText(JSON.stringify(snapshot, null, 2));
    const unmatched = Array.isArray(snapshot.mappingDiagnostics)
      ? snapshot.mappingDiagnostics.filter((item) => item.state === "unmatched").length
      : 0;
    setStatus(`已复制脱敏调试日志：扫描 ${snapshot.scan?.fieldCount || 0} 个字段，未匹配 ${unmatched} 个。可直接粘贴给 Codex 分析。`);
  } catch (error) {
    setStatus(`复制调试日志失败：${error.message}`, true);
  }
}

async function clearDebugLog() {
  try {
    const response = await sendRuntimeMessage({ type: "OJAF_CLEAR_DEBUG_HISTORY" });
    setStatus(`已清除 ${response?.count || 0} 条本机调试日志。`);
  } catch (error) {
    setStatus(`清除调试日志失败：${error.message}`, true);
  }
}
