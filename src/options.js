const fields = {
  ...
  profileBackupHistory: document.getElementById("profileBackupHistory"),
  restoreProfileBackup: document.getElementById("restoreProfileBackup"),
  downloadProfileBackupNow: document.getElementById("downloadProfileBackupNow"),
...
fields.autoBackupEnabled.addEventListener("change", updateAutoBackupSetting);
fields.profileBackupHistory.addEventListener("change", updateRestoreBackupButton);
fields.restoreProfileBackup.addEventListener("click", restoreSelectedProfileBackup);
fields.downloadProfileBackupNow.addEventListener("click", downloadDurableProfileBackup);
...
async function loadSettings() {
  try {
    ...
    applyBackupConfig(settings.backupConfig);
    await refreshProfileBackupHistory();
...
async function saveProfile() {
  try {
    ...
    const result = await sendRuntimeMessage({
      type: "OJAF_SAVE_SETTINGS",
      payload: { profileV2, profileSaveSource: "settings" }
    });
    await refreshProfileBackupHistory();
    const backupNote = formatAutomaticBackupResult(result?.backup);
    setStatus(`简历资料保存成功，已保存到本机浏览器。${backupNote}`);
    setProfileSaved(`资料已保存到本机，并加入版本历史。${backupNote}`);
    showToast(result?.backup?.saved ? "资料已保存并持久备份。" : "资料已保存到本机。");
  } catch (error) {
...
async function exportProfile() {
  try {
    ...
    anchor.href = url;
    anchor.download = `openjobautofill-profile-${formatBackupFilenameTimestamp(new Date())}.json`;
...
function applyBackupConfig(config = {}) {
  fields.autoBackupEnabled.checked = Boolean(config.autoDownloadEnabled);
  updateBackupStatus(config);
}

async function refreshProfileBackupHistory() {
  const state = await sendRuntimeMessage({ type: "OJAF_GET_PROFILE_BACKUPS" });
  const history = Array.isArray(state?.history) ? state.history : [];
  fields.profileBackupHistory.replaceChildren();

  if (history.length === 0) {
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "还没有历史版本；保存资料后会自动创建";
    fields.profileBackupHistory.append(option);
  } else {
    for (const entry of history) {
      const option = document.createElement("option");
      option.value = entry.id;
      option.textContent = `${formatBackupDisplayTime(entry.savedAt)} · ${formatBackupSource(entry.source)}`;
      fields.profileBackupHistory.append(option);
    }
  }

  fields.profileBackupHistory.disabled = history.length === 0;
  updateRestoreBackupButton();
  applyBackupConfig(state?.config || {});
}

function updateRestoreBackupButton() {
  fields.restoreProfileBackup.disabled = !fields.profileBackupHistory.value;
}

async function updateAutoBackupSetting() {
  const requestedEnabled = fields.autoBackupEnabled.checked;
  fields.autoBackupEnabled.disabled = true;
  try {
    if (requestedEnabled) {
      const granted = await requestOptionalPermissions({ permissions: ["downloads"] });
      if (!granted) {
        throw new Error("未授予下载权限，自动持久备份没有开启。");
      }
    }

    const config = await sendRuntimeMessage({
      type: "OJAF_SET_BACKUP_CONFIG",
      payload: { autoDownloadEnabled: requestedEnabled }
    });
    fields.autoBackupEnabled.checked = Boolean(config.autoDownloadEnabled);

    if (config.autoDownloadEnabled) {
      const result = await sendRuntimeMessage({ type: "OJAF_DOWNLOAD_PROFILE_BACKUP" });
      if (!result?.saved) {
        throw new Error("自动备份已开启，但首次持久备份没有写入成功。");
      }
      await refreshProfileBackupHistory();
      setBackupStatus(`自动备份已开启；已写入 ${result.filename}。`, "saved");
      showToast("自动持久备份已开启。");
    } else {
      setBackupStatus("自动持久备份已关闭；浏览器内仍会保留最近 10 个不同版本。");
    }
  } catch (error) {
    fields.autoBackupEnabled.checked = false;
    await sendRuntimeMessage({
      type: "OJAF_SET_BACKUP_CONFIG",
      payload: { autoDownloadEnabled: false }
    }).catch(() => undefined);
    setBackupStatus(error.message, "error");
    showToast(error.message, "error");
  } finally {
    fields.autoBackupEnabled.disabled = false;
  }
}

async function downloadDurableProfileBackup() {
  fields.downloadProfileBackupNow.disabled = true;
  try {
    const granted = await requestOptionalPermissions({ permissions: ["downloads"] });
    if (!granted) {
      throw new Error("未授予下载权限，无法写入持久备份目录。");
    }

    const result = await sendRuntimeMessage({ type: "OJAF_DOWNLOAD_PROFILE_BACKUP" });
    if (!result?.saved) {
      throw new Error("持久备份没有写入成功。");
    }
    await refreshProfileBackupHistory();
    setBackupStatus(`已写入 ${result.filename}。该文件会在移除扩展后继续保留。`, "saved");
    showToast("持久备份已写入下载目录。");
  } catch (error) {
    setBackupStatus(error.message, "error");
    showToast(error.message, "error");
  } finally {
    fields.downloadProfileBackupNow.disabled = false;
  }
}

async function restoreSelectedProfileBackup() {
  const backupId = fields.profileBackupHistory.value;
  if (!backupId) {
    return;
  }

  const selectedLabel = fields.profileBackupHistory.selectedOptions[0]?.textContent || "所选版本";
  const confirmed = window.confirm(`确定恢复“${selectedLabel}”吗？当前编辑区内容会被替换；当前已保存版本仍会留在历史中。`);
  if (!confirmed) {
    return;
  }

  fields.restoreProfileBackup.disabled = true;
  try {
    const result = await sendRuntimeMessage({
      type: "OJAF_RESTORE_PROFILE_BACKUP",
      payload: { backupId }
    });
    renderProfileSectionEditor(result.profileV2);
    setProfileSaved("已恢复所选历史版本并保存到本机。");
    await refreshProfileBackupHistory();
    setStatus(`已恢复 ${selectedLabel}。${formatAutomaticBackupResult(result?.backup)}`);
    showToast("历史版本已恢复。");
  } catch (error) {
    setBackupStatus(`恢复失败：${error.message}`, "error");
...
function updateBackupStatus(config = {}) {
  if (config.lastExternalBackupAt && config.lastExternalBackupFilename) {
    setBackupStatus(
      `最近持久备份：${formatBackupDisplayTime(config.lastExternalBackupAt)} · ${config.lastExternalBackupFilename}`,
      "saved"
    );
    return;
  }
  ...
  setBackupStatus(
    config.autoDownloadEnabled
      ? "自动备份已开启；下次保存资料时会写入下载目录。"
      : "浏览器内会保留最近 10 个不同版本；建议开启自动备份以防扩展被移除。"
  );
}

function setBackupStatus(message, state = "") {
  fields.backupStatus.textContent = message;
  fields.backupStatus.classList.toggle("is-saved", state === "saved");
  fields.backupStatus.classList.toggle("error", state === "error");
}

function formatAutomaticBackupResult(result) {
  if (!result) {
    return "";
  }
  if (result.saved) {
    return ` 已自动备份到 ${result.filename}。`;
  }
  if (result.reason === "permission-required") {
    return " 自动备份权限已失效，请在备份保险中重新开启。";
  }
  return " 自动持久备份未写入，但浏览器内版本历史已保留。";
}
...
function formatBackupSource(source) {
  const labels = {
    ...
    import: "导入备份",
    "inline-edit": "页面内编辑",
    "before-restore": "恢复前版本",
    save: "资料保存"
  };
  return labels[source] || "资料保存";
}
...
async function importProfileFromFile() {
  ...
  try {
    ...
    const result = await sendRuntimeMessage({
      type: "OJAF_SAVE_SETTINGS",
      payload: { profileV2, profileSaveSource: "import" }
    });
    await refreshProfileBackupHistory();
    setStatus(`已导入并保存到本机。资料面板会立即读取这份简历资料。${formatAutomaticBackupResult(result?.backup)}`);
...
async function clearLocalData() {
  const confirmed = window.confirm("这会清空当前浏览器保存的简历资料、API 设置和内部版本历史。已下载的独立备份不会删除。是否继续？");
  if (!confirmed) {
    return;
  }
...
function renderStructuredSection(section, data = null) {
  ...
  const sectionDescription = config.isExtra
    ...
    : isRepeat
      ...
      : "直接填写字段；没有内容可以留空。";
  const matchAliasDescription = Array.isArray(config.matchAliases) && config.matchAliases.length > 0
    ? ` 匹配别名：${config.matchAliases.join("、")}。`
    : "";
    ...
    <section id="profile-section-${escapeHtml(section.key)}" class="profile-edit-section" data-profile-section="${escapeHtml(section.key)}" data-section-title="${escapeHtml(section.title)}" data-extra-section="${config.isExtra ? "true" : "false"}">
      <div class="profile-edit-head">
        <div>
          ...
          <p>${escapeHtml(`${sectionDescription}${matchAliasDescription}`)}</p>
        </div>
        ${action}
      </div>
      ${body}
